...
import { BarChart, Bar, XAxis, YAxis, Tooltip, Legend, ResponsiveContainer, PieChart, Pie, Cell } from 'recharts'; // Removi 'Sector' que não estava sendo usado
import { AppContext } from '../context/AppContext';
import { alpha } from '@mui/material/styles';

// --- IMPORTANTE: Usando o serviço de API centralizado ---
...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        // ALTERAÇÃO AQUI: Usando 'api.get' sem precisar digitar a URL inteira
                ...
                {recentes.map((item, index) => (
                  <React.Fragment key={item.id}>
                    <ListItem disablePadding>
                      <ListItemAvatar>
                        {/* Ajuste para garantir que a imagem venha da API corretamente */}
                        <Avatar variant="rounded" src={item.imagem_url || '/placeholder.png'} />
                      </ListItemAvatar>
                      <ListItemText primary={item.nome} secondary={item.codigo_produto} />
                    </ListItem>
