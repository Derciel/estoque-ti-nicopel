  ...
  return (
    <Paper sx={{ p: 3, overflow: 'hidden' }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 2 }}>
        ...
        <Button
          variant="contained"
          startIcon={<PictureAsPdfIcon sx={{ color: selecionados.length > 0 ? '#0B0F1A' : 'inherit' }} />}
          onClick={handleExportarPDF}
          disabled={selecionados.length === 0}
          sx={{
            fontWeight: 800,
            boxShadow: selecionados.length > 0 ? '0 8px 16px -4px rgba(190, 242, 100, 0.4)' : 'none',
            '&.Mui-disabled': {
        ...
        <Table size="small">
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={{ display: { xs: 'none', lg: 'table-cell' } }}>Usuário</TableCell>
              <TableCell align="center">Status</TableCell>
              <TableCell align="center" sx={{ display: { xs: 'none', sm: 'table-cell' } }}>QR Code</TableCell>
                ...
                <TableRow key={item.id} hover selected={isSelected}>
                  ...
                  <TableCell>
                    <Avatar
                      ...
                      sx={{ cursor: item.imagem_url ? 'pointer' : 'default', width: 40, height: 40 }}
                    />
                  </TableCell>
                  <TableCell sx={{ display: { xs: 'none', md: 'table-cell' } }}>{item.id}</TableCell>
                  <TableCell sx={{ fontWeight: 600 }}>{item.nome_produto}</TableCell>
                  <TableCell>{item.codigo_produto}</TableCell>
                  <TableCell sx={{ display: { xs: 'none', lg: 'table-cell' } }}>{item.nome_usuario || '---'}</TableCell>
                  <TableCell align="center"><Chip label={item.status} color={getStatusChipColor(item.status)} size="small" sx={{ fontWeight: 700, fontSize: '0.7rem' }} /></TableCell>
                  <TableCell align="center" sx={{ p: '4px', display: { xs: 'none', sm: 'table-cell' } }}>
                    <Box sx={{ p: 0.5, backgroundColor: 'white', borderRadius: 1, display: 'inline-block' }}>
                      <QRCodeCanvas value={getQrCodeValue(item)} size={40} bgColor={"#ffffff"} fgColor={"#000000"} imageSettings={{ src: "/logo.png", height: 10, width: 10, excavate: true }} />
                    </Box>
                  </TableCell>
